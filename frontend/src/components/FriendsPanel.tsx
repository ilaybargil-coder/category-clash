...
export default function FriendsPanel() {
  ...
  return (
    <section className="surface-panel rounded-2xl p-5 sm:p-6">
      ...
      {error && (
        <p className="mt-3 rounded-xl border border-rose-400/20 bg-rose-500/10 p-3 text-sm text-rose-300">{error}</p>
      )}
      ...
      {requests.incoming.length > 0 && (
        <div className="mt-5">
          <div className="mb-3 flex items-center gap-2">
            <h3 className="font-black text-white">בקשות חברות</h3>
            <span className="rounded-full bg-violet-500/15 px-2 py-0.5 text-xs font-black text-violet-300">
      ...
      <div className="mt-5">
        ...
        {loading ? (
          ...
          <p className="rounded-xl border border-white/10 bg-white/[0.025] p-4 text-center text-sm text-slate-400">
...
function RelationButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="primary-button shrink-0 px-4 py-2 text-sm disabled:text-slate-400"
    >
